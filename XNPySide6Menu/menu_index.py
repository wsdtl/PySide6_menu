...
class MenuLeftSideFirst(PushButton):
    ...
    参数:
        text (str): 菜单文本\n
        icon (QPixmap): 菜单图标\n
        font_name (Optional[str] = 'SimHei'): 字体名字\n
        font_size (Optional[int] = 14): 字体大小\n
        theme_color (Optional[QColor] = QColor(255, 255, 255, 255)): 点击后颜色
    """
    _w: int = 170
    _h: int = 50
...
class MenuLeftSideSecondary(PushButton):
    ...
    参数:
        ...
        font_name (Optional[str] = 'SimHei'): 字体名字\n
        font_size (Optional[int] = 12): 字体大小
    """
    _w: int = 170
    _h: int = 50
    ...
    def draw(
        ...
        参数:
            ...
            painter (QPainter): QPainter\n
            font_color (str): 字体颜色\n
            icon (Optional[bool], optional): 二级菜单是否按下标志. Defaults to False.
        """
        if icon:
            painter.drawPixmap(
                self._move_letf,
